...
for TARGET in "${TARGETS[@]}"; do
  ...
  if [ "$FORCE_EVAL" = "1" ] || [ ! -f "$RESULT_FILE" ]; then
    echo ""
    echo "================ DISTILBERT VIB-HYBRID BAYES EVAL: target=${TARGET} ================"
    python eval/eval_bayes_vib.py \
      ...
      --vib_risk_control_require_disagreement "$VIB_RISK_CONTROL_REQUIRE_DISAGREEMENT" | tee "logs/eval_bayes_vib_hybrid_distilbert_${TARGET}.log"
  else
...
echo ""
echo "================ DISTILBERT VIB-HYBRID SCORING SUMMARY ================"
...
row = {"Router": "distilbert_vib_hybrid"}
