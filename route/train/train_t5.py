...
def main():
    ...
    parser.add_argument("--report_to", type=str, default="none", help="Integration to report metrics to")
    parser.add_argument("--gradient_accumulation_steps", type=int, default=1, help="Gradient accumulation steps")
    parser.add_argument("--gradient_checkpointing", action="store_true", help="Enable gradient checkpointing")
    parser.add_argument("--mixed_precision", type=str, default="auto", choices=["auto", "fp16", "bf16", "no"], help="Precision mode")
    ...
    sources = ["no", "paragraph", "document", "image"]
    class_label = ClassLabel(names=sources)

    dataset = dataset.filter(
        lambda x: x["source"] in sources and x["gt_retrieval"] in sources
    )
