...
def main(input_path, output_path, batch_size=128, max_input_length=512):
    ...
    for path in input_path:
        ...
        for i in tqdm(
            ...
            desc=f"Constrained-routing {os.path.basename(path)} with {model.config._name_or_path}",
        ):
            batch_questions = questions[i : i + batch_size]
            batch_retrievals, batch_probabilities = route(batch_questions, max_input_length=max_input_length)
...
if __name__ == "__main__":
    ...
    model = T5ForConditionalGeneration.from_pretrained(args.checkpoint_dir, torch_dtype=torch.bfloat16).to(device)
    model.eval()

    input_path = [os.path.join(args.input_dir, fname) for fname in os.listdir(args.input_dir) if fname.endswith(".json")]
    model_size = args.output_model_name or os.path.basename(args.checkpoint_dir)
